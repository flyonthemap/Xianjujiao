...
public class NewsAdapter extends BaseAdapter {
    ...
    private List<NewsData.NewItem> chapterListItems;
    private LayoutInflater mLayoutInflater;

    public NewsAdapter(Context context, List<NewsData.NewItem> chapterListItems) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        NewsData.NewItem chapterListItem = chapterListItems.get(position);
        viewHolder.title.setText(chapterListItem.title);
        //格式化时间

        String time = DateUtils.dateFormat(chapterListItem.create_time);
        viewHolder.date.setText(time);//文章发布时间
//        viewHolder.comment.setText(chapterListItem.getFeedback());//评论数
        viewHolder.tv_id.setText(chapterListItem.news_id);//文章id
        viewHolder.tv_typeId.setText(chapterListItem.type+"");//文章分类id
//        viewHolder.tv_url.setText(chapterListItem.getArcurl());//文章URl
        ...
        //获取到图片地址
        String litpic = chapterListItem.thumb_image;
        //如果图片地址为空，则设置默认图片
        if (litpic == null) {
        ...
        //下载图片，优先使用本地缓存图片
        x.image().bind(iv,chapterListItem.thumb_image);
        return convertView;
    }
